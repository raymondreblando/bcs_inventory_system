...
if(empty($firstname) OR empty($lastname) OR empty($gender) OR empty($phone_number) OR empty($email_address) OR empty($address)){
       ...
       $functions->notification("Invalid email address.", "error", 2500, "");
        ...
        $database->DBQuery("SELECT `user_id`,`email`,`photo` FROM `users` WHERE `user_id` = ?",[$_SESSION['uid']]);
        $user = $database->fetch();
       ...
       if($check_email_exist == 1){
              if (!empty($_FILES["user_profile"])) {
                     ...
                     if(!in_array($getFileExt, $allowTypes)){
                            ...
                            $database->DBQuery("UPDATE `users` SET `email`=?,`photo`=?,`fname`=?,`middle`=?,`lname`=?,`gender`=?,`contact`=?,`address`=? WHERE `user_id` = ?",[$email_address, $newFilename, $firstname, $middlename, $lastname, $gender, $phone_number, $address, $_SESSION['uid']]);

                            $functions->notification("Profile successfully updated.", "success", 1500, "yes");
                     
                     }
              }else{
                     $database->DBQuery("UPDATE `users` SET `email`=?,`fname`=?,`middle`=?,`lname`=?,`gender`=?,`contact`=?,`address`=? WHERE `user_id` = ?",[$email_address, $firstname, $middlename, $lastname, $gender, $phone_number, $address, $_SESSION['uid']]);
